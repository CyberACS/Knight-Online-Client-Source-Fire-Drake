...
#define SLAVE_MERCHANT				751730000

#define AUTOMATIC_DROP_MINING		751690000
#define AUTOMATIC_DROP_FISHING		751710000 
////////////////////////////////////////////////////////////
...
// Bowl Event MeLanor
struct _MONSTER_BOWL_TABLE
{
	uint16_t sIndex;
	uint32_t Cashbonus, sEffect;
};


struct _CYBERACS_COLLECTIONRACE_SETTINGS
{
	uint32  ID;
	uint16  CREventMaxUser;
	uint32  CREventItemID[5];
	uint32	CREventWinEXP;
	uint32	CREventWinCoin;
	uint32	CREventWinNP;
	uint32 CREventWinKC;
	std::string CREventstrName[5];
	uint32  CREventItemCount[5];
	uint32  CREventItemTimed[5];
	uint8   CREventMaxLevel;
	uint8	CREventMinLevel;
};

struct _MONSTER_SEARCH_ITEM
{
	uint32  ID;
	std::string Name;
	std::string ZoneName;
...
struct _CYBERACS_MANNER_STORE_ITEM
{
	uint32 sItemID;
	uint32 sPrice;
	uint32 sBuyCount;
...
struct _CYBERACS_LOTTERY_SETTINGS
{
	uint32  ID;
	uint32  NumReward[3];
	uint32  EventItemCount[3];
	uint32  EventItemTimed[3];
};

struct _CYBERACS_COLLECTIONRACE_HUNT_LIST
{
	uint8	QuestID;
	uint8	QuestZone;
	uint16	MonsterNum1;
...
struct _CYBERACS_DAILY_SETTINGS
{
	uint32  ID;
...
struct _CYBERACS_DAILY_HUNT_LIST
{
	uint8	QuestID;
...
struct _CYBERACS_DAILY_USER_QUEST_LIST
{
	uint8	Quest[30];
};
...
struct _TOURNAMENT_SCHEDULE
{
	uint8 EventLocalID;
	std::string Start;
	uint8 EventStatus;
	uint32 EventStartHour;
	uint32 EventStartMinutes;
	uint8 TimeActive;
};
...
struct _WANTED_MAIN_CONTROL
{
	bool   ActiveEvent;
	bool   ActiveQuery;
	bool   ActiveInfoList;
	bool   EventisPassive;
	bool   EventDangerControl;
	int8   EventZoneID;
	uint8  RemainKarusUser;
	uint8  RemainElmoradUser;
	uint32 EventStartTime;
	uint32 EventActiveTime;
	uint32 ActiveQueryTime;
	uint32 EventUserListInfoTime;

	_WANTED_MAIN_CONTROL()
	{
		Initialize();
	}
	...
	void Initialize()
	{
		ActiveEvent = false;
		ActiveQuery = false;
		ActiveInfoList = false;
		EventisPassive = true;
		EventDangerControl = false;
		EventZoneID = -1;
		RemainKarusUser = 0;
		RemainElmoradUser = 0;
		EventStartTime = 60;
		EventActiveTime = 0;
		ActiveQueryTime = 0;
		EventUserListInfoTime = 0;
	}
};

struct _WANTED_EVENT_SELECTED_USER
{
	uint16 UserGetID;
	uint16 UserGetZoneID;
	uint8  UserGetNation;
	bool  UserisWanted;
};
...
struct _USER_RANK_ALL
{
	uint16  ID;
	std::string  strUserId;
	uint8  Nation;
	uint32  Loyalty;
	uint32	LoyaltyMonthly;
...
struct _CYBERACS_SERVER_SETTINGS
{
	uint32 nServerNo;
...
struct _CYBERACS_SETTINGS
{
	...
	uint8 Button_Youtube;
	uint8 Button_Twitter;
	uint8 Button_Skype;
	std::string URL_Facebook;
	std::string URL_Discord;
	std::string URL_Live;
	std::string URL_Youtube;
	std::string	URL_Twitter;
	std::string URL_Skype;
