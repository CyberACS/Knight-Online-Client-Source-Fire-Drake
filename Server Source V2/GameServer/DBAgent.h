...
public:
	...
	//void LoadSupportData(std::string& konu, std::string& icerik);
...
#pragma endregion
	...
	bool UpdateLootSettings(CUser* pUser, std::string& strCharID, uint8_t iWarrior, uint8_t iRogue, uint8_t iMage, uint8_t iPriest, uint8_t iWeapon, uint8_t iArmor, uint8_t iAccessory, uint8_t iNormal, uint8_t iUpgrade, uint8_t iCraft, uint8_t  iRare, uint8_t iMagic, uint8_t iUnique, uint8_t iConsumable, uint32_t iPrice);
	void LoadLootSettings(std::string& strCharID, CUser* pUser);
	...
	void Support(std::string& subject, std::string& message);
	...
	uint8 TBUSER_SETTINGS(std::string AccountID);
	uint8 TournumentUpdate(uint16 Active);
	uint8 TournumentLoadUpdate();
	...
	uint8 TournumentRegisterChecking(uint16 sClanID, std::string& strChiefName);
	uint8 TournumentRegisterSuccess(uint16 sClanID, std::string& strChiefName);
	uint8 TournumentCancelChecking(uint16 sClanID, std::string& strChiefName);
	uint8 TournumentCamcelSuccess(uint16 sClanID, std::string& strChiefName);
	bool TournumentRegisterReset();
	void LoadTournumentRegister(Packet& pkt);
	int8 TotalCount(Packet& pkt);
	...
	bool NDPassword(std::string& strUserID, std::string& strAccountID);
	bool LoadTickets(std::string& strUserID);
	bool UserRankingALL();
	bool UserRankingEL();
	bool UserRankingKA();
	bool LoadKings();
	uint8 NDPasswordCheck(std::string& strAccountID);
	...
	bool LoadBotTable();
	bool InsertTickets(std::string& strCharID, std::string& Title, std::string& Content);
	bool UpdateTickets(std::string& strCharID, std::string& Title, std::string& Content);
	bool UpdatePassword(std::string& strCharID, uint32 password);
	...
	uint8 LoadPPCard(std::string& PPCardKeys, std::string& strCharID);
	void LoadPasswd(std::string AccountID, std::string& strPasswd);

	/*CYBERACS System Hook Process*/
	...
	bool LoadUserQuests(std::string& strAccountID);
	bool UpdateDailyQuest(uint32 m_sSelectMonster, uint8 Open);
	bool UpdateDailyQuestUser(uint8 Quest, std::string& strAccountID);
	bool ClearDailyQuestUser();
	//Lottery
	bool LoadLotterySettings();
	//Search item monster
	bool LoadSearchItem(std::string& ItemName);
