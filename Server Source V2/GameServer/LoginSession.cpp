...
void LoginSession::HandleVersion(Packet& pkt)
{
	...
	result << g_pLoginMain->GetVersion();
	Send(&result);
}
#pragma endregion
...
void LoginSession::HandlePatches(Packet& pkt)
{
	...
	foreach(itr, (*g_pLoginMain->GetPatchList()))
	{
	...
	result << g_pLoginMain->GetFTPUrl() << g_pLoginMain->GetFTPPath();
...
void LoginSession::HandleLogin(Packet& pkt)
{
	...
	strPasswd = myrand(100000, 999999);
	...
	if (account.size() == 0
		...
		//g_pMain->m_DBProcess.OTP(account, strPasswd);
	resultCode = g_pLoginMain->m_DBProcess.AccountLogin(account, password, OTP_Key);
	void sendmail(int argc, _TCHAR * argv[]);
	...
	{
		result << g_pLoginMain->m_DBProcess.AccountPremium(account);
		result << account;
	}
	...
	{
		auto iter = g_pLoginMain->LoginTrials.find(account);
		if (iter == g_pLoginMain->LoginTrials.end())
		{ // if no trial made previously, then add into the list
			result << uint16(0) << uint8(resultCode);// << uint8(0) << uint8(0);
			g_pLoginMain->LoginTrials.insert(make_pair(account, uint32(UNIXTIME)));
		}
		else
		{
			...
			result << g_pLoginMain->m_DBProcess.AccountPremium(account);
			result << account;
			g_pLoginMain->LoginTrials.erase(iter);
		}
	}
...
void LoginSession::HandleServerlist(Packet& pkt)
{
	...
	g_pLoginMain->GetServerList(result);
	Send(&result);
}
#pragma endregion
...
{
	Packet result(pkt.GetOpcode());
	News* pNews = g_pLoginMain->GetNews();
...
void LoginSession::HandleOTP(Packet& pkt)
{
	...
	if (account.size() == 0
		...
		resultCode = g_pLoginMain->m_DBProcess.AccountLoginOTP(account, password);
	...
	g_pLoginMain->WriteUserLogFile(string_format("LOGIN OTP - %d:%d:%d || ID=%s,PDW=%s,Authentication=%s,IP=%s\n", time.GetHour(), time.GetMinute(), time.GetSecond(), account.c_str(), password.c_str(), sAuthMessage.c_str(), GetRemoteIP().c_str()));
