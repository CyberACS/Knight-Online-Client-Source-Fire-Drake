...
void Unit::SendToRegion(Packet* result)
{
	...
	if (!isNPC() && pPriest != nullptr)//bot ise
	{
		CBot* pPriest = nullptr;
		pPriest = g_pMain->m_BotArray.GetData(GetID());
		if(pPriest != nullptr && !pPriest->isNPC())
			pPriest->SendToRegion(result);
	}
	else
...
void KOMap::UpdateDeathMatch(bool Open, ZoneAbilityType type)
{
	if (g_pMain->pTempleEvent.isActive == true)
	{
		m_zoneType = type;
		m_zoneFlags = ZF_ATTACK_OTHER_NATION | ZF_ATTACK_SAME_NATION | ZF_FRIENDLY_NPCS;
	}
...
void KOMap::UpdateMap(bool Open, ZoneAbilityType type)
{
	if (Open)
	{
		m_zoneType = type;
...
void KOMap::UpdateVsEventMap(bool Open, ZoneAbilityType type)
{
	if (Open)
	{
		m_zoneType = type;
		m_zoneFlags = ZF_ATTACK_OTHER_NATION | ZF_ATTACK_SAME_NATION | ZF_TALK_OTHER_NATION;
	}
	else
	{
...
bool CUser::isHostileTo(Unit * pTarget)
{
	...
	if (GetZoneID() == ZONE_DEATHMATCH && TO_USER(pTarget)->GetZoneID() == ZONE_DEATHMATCH)
	{
		if (GetName() == pTarget->GetName())
			return false;
		else
	...
	if (GetZoneID() == ZONE_CLAN_WAR_RONARK && TO_USER(pTarget)->GetZoneID() == ZONE_CLAN_WAR_RONARK)
	{
		if (GetName() == pTarget->GetName())
			return false;
		else
...
short CBot::GetDamage(Unit* pTarget, _MAGIC_TABLE* pSkill /*= nullptr*/, bool bPreviewOnly /*= false*/)
{
	...
	*/
	...
	if (pTarget->isPlayer())
	{
		CBot* pTUser = TO_USERB(pTarget);	// NOTE: using a = a*v instead of a *= v because the compiler assumes different 
		// types being multiplied, which results in these calcs not behaving correctly.
	...
	if (pTarget->isPlayer())
	{
			...
			&& TO_USERB(pTarget)->isRogue())
		{
			...
			&& pTarget != nullptr
			&& pTarget->isPlayer()
			&& (TO_USERB(pTarget)->isPortuKurian()))
		{
			damage /= 2;
			double dm = (double)damage;
			dm *= 1.10f;
...
bool CBot::isHostileTo(Unit* pTarget)
{
	...
	// Players can attack other players in the Moradon arena.
	if (isInArena() && TO_USERB(pTarget)->isInArena())
	{
		if ((GetX() < 735.0f && GetX() > 684.0f)
			&& (GetZ() < 411.0f && GetZ() > 360.0f)) // party arena in moradon
		{
			if (isInParty() && pTarget->isPlayer()
				&& GetPartyID() == TO_USERB(pTarget)->GetPartyID())
				return false;
			else
				return true;
		}
	...
	if (GetZoneID() == ZONE_DEATHMATCH && TO_USERB(pTarget)->GetZoneID() == ZONE_DEATHMATCH)
	{
		if (GetName() == pTarget->GetName())
			return false;
		else
	...
	if (GetZoneID() == ZONE_CLAN_WAR_RONARK && TO_USERB(pTarget)->GetZoneID() == ZONE_CLAN_WAR_RONARK)
	{
		if (GetName() == pTarget->GetName())
			return false;
		else
	...
	if (GetZoneID() == ZONE_ARENA)
	{
		if (isInRangeSlow(64.0f, 178.0f, 60)
			|| isInRangeSlow(192.0f, 178.0f, 60)) // we are at the rose clan arena
		{
			if (!isInClan() || !TO_USERB(pTarget)->isInClan())
				return false;

			if (isInClan() && TO_USERB(pTarget)->isInClan()
				&& GetClanID() == TO_USERB(pTarget)->GetClanID())
				return false;
		}
	...
	// Players can attack other players in the safety area.
	if (GetNation() != pTarget->GetNation()
		&& TO_USERB(pTarget)->isInOwnSafetyArea())
		return false;
	...
	if (GetZoneID() == ZONE_DELOS)
	{
		if (g_pMain->pCswEvent.Started)
		{
			if (!isInOwnSafetyArea() && !TO_USERB(pTarget)->isInOwnSafetyArea())
			{
				CKnights* pKnightsSource = g_pMain->GetClanPtr(GetClanID());
				CKnights* pKnightsTarget = g_pMain->GetClanPtr(TO_USERB(pTarget)->GetClanID());
				if (pKnightsSource != nullptr && pKnightsTarget != nullptr)
				{
					_KNIGHTS_SIEGE_WARFARE* pKnightSiege = g_pMain->GetSiegeMasterKnightsPtr(Aktive);
					if (pKnightSiege != nullptr)
					{
						...
						{
							_DEATHMATCH_WAR_INFO* pDeathmatchSource = g_pMain->m_KnightSiegeWarClanList.GetData(GetClanID());
							_DEATHMATCH_WAR_INFO* pDeathmatchTarget = g_pMain->m_KnightSiegeWarClanList.GetData(TO_USERB(pTarget)->GetClanID());
							if (pDeathmatchSource != nullptr && pDeathmatchTarget != nullptr)
							{
								if (GetClanID() != TO_USERB(pTarget)->GetClanID())
									return true;
							}
						}
						else if (g_pMain->pCswEvent.Status == CswOperationStatus::CastellanWar)
						{
							...
							if (pOldWinnerSource != nullptr)
							{
								_CASTELLAN_WAR_INFO* pInfo1 = g_pMain->m_KnightSiegeWarCastellanClanList.GetData(TO_USERB(pTarget)->GetClanID());
								if (pInfo1 != nullptr
									&& (GetClanID() != TO_USERB(pTarget)->GetClanID()))
									return true;
							}
							...
							if (pInfo1 != nullptr && GetClanID() != pKnightSiege->sMasterKnights)
							{
								_CASTELLAN_OLD_CSW_WINNER* pOldWinnerTarget = g_pMain->m_KnightSiegeWarCastellanOldWinner.GetData(TO_USERB(pTarget)->GetClanID());
								if (pOldWinnerTarget != nullptr)
									return true;
								else
								{
									if (TO_USERB(pTarget)->GetClanID() == pKnightSiege->sMasterKnights)
										return true;
								}
							}
						}
