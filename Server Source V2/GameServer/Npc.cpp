...
void CNpc::OnDeathProcess(Unit *pKiller)
{
	...
	else if (m_bMonster)
	{
			...
			if (pMonsterRespawn != nullptr)
			{
				if (pMonsterRespawn->sZoneID != 0)
				{
					if (pMonsterRespawn->sZoneID == GetZoneID())
						g_pMain->SpawnEventNpc(pMonsterRespawn->sSid, pMonsterRespawn->sType == 0 ? true : false, GetZoneID(), GetX(), GetY(), GetZ(), pMonsterRespawn->sCount, pMonsterRespawn->sRadius, pMonsterRespawn->isDeadTime * 60, 0, GetID(), pUser->GetEventRoom(), 0, 1, 0);
				}
				else
				{
					g_pMain->SpawnEventNpc(pMonsterRespawn->sSid, pMonsterRespawn->sType == 0 ? true : false, GetZoneID(), GetX(), GetY(), GetZ(), pMonsterRespawn->sCount, pMonsterRespawn->sRadius, pMonsterRespawn->isDeadTime* 60, 0, GetID(), pUser->GetEventRoom(), 0, 1, 0);
		...
		if (g_pMain->m_CyberACSCollectionRaceGameServerSatus)
			pUser->CyberACSCollectionRaceProcess(GetProtoID());

				pUser->DailyProcess(GetProtoID());
...
bool CNpc::Kontrol(uint16 sNpcID)
{
	/*CNpc* pNpc = g_pMain->GetNpcPtr(GetProtoID(), GetZoneID());
	if (sNpcID > 32095) {
		g_pMain->KillNpc(sNpcID, GetZoneID(), this);
		CNpcThread* zoneitrThread = g_pMain->m_arNpcThread.GetData(GetZoneID()); 
		zoneitrThread->m_arNpcArray.DeleteData(sNpcID);
		delete pNpc;
	}
		*/
	return false;
...
void CNpc::ChangeTarget(int nAttackType, CUser *pUser)
{
	...
	if (pUser->isMonsterTransformation() && !pUser->attacknpc)
	{
		pUser->attacknpc = true;
		pUser->m_npcattack = UNIXTIME + 10;
	}
...
int CNpc::GetTargetPath(int option)
{
	...
	if (m_Target.id < NPC_BAND)
	{
		...
		if (pUser == nullptr
			...
			|| (pUser->isMonsterTransformation() && !pUser->attacknpc))
			return -1;
...
time_t CNpc::LongAndMagicAttack()
{
	...
	if (nID < NPC_BAND)
	{
		...
		std:string pFirstClan = g_pMain->clanlar[i];
		string pSecondClan = g_pMain->clanlar[i + 1];
		pUser = g_pMain->GetUserPtr(nID);
		CKnights* clan = g_pMain->GetClanPtr(pUser->GetClanID());
		
		if (pUser == nullptr
				|| pUser->isDead()
				|| pUser->m_bInvisibilityType
				|| GetProtoID() == 5312 && pFirstClan == clan->m_strName
				|| GetProtoID() == 5313 && pSecondClan == clan->m_strName
				|| pUser->isGM())
			{
				InitTarget();
				StateChange(NPC_STANDING);
...
void CNpc::GiveNpcHaveItem(Unit *pKiller)
{
	...
	if (isMonster())
	{
		...
		if (pItem != nullptr)
		{
			...
			{
				//_ITEM_TABLE* pTable = g_pMain->GetItemPtr(pItem->iItem[j]);
				if (pItem->iItem[j] == 0 
					|| pItem->sPercent[j] == 0)
					continue;
	...
	for (int i = 0; i < nCount; i++)
	{
		if (g_pMain->GetItemPtr(m_GiveItemList[i].sSid))
		{
			...
			_ITEM_TABLE* pTable = g_pMain->GetItemPtr(m_GiveItemList[i].sSid);

			pTable->m_ItemType;

			pUser->m_lootSettings.iNormal;

			if (pTable->m_bKind == 230 || pTable->m_bKind == 240 || pTable->m_bKind == 220 || pTable->m_bKind == 210 && pUser->m_lootSettings.iArmor != 1)
				continue;
			if (pTable->m_bKind == 70 || pTable->m_bKind == 71 || pTable->m_bKind == 11 || pTable->m_bKind == 62 || pTable->m_bKind == 31 || pTable->m_bKind == 21 || pTable->m_bKind == 41 || pTable->m_bKind == 51 || pTable->m_bKind == 181
				|| pTable->m_bKind == 60 || pTable->m_bKind == 110 || pTable->m_bKind == 32 || pTable->m_bKind == 42 || pTable->m_bKind == 52 || pTable->m_bKind == 22 && pUser->m_lootSettings.iWeapon != 1)
				continue;
			if (pTable->m_ItemType == 0 && pUser->m_lootSettings.iNormal != 1)
				continue;
			if (pTable->m_ItemType == 5 && pUser->m_lootSettings.iUpgrade != 1)
				continue;
			if (pTable->m_ItemType == 3 && pUser->m_lootSettings.iCraft != 1)
				continue;
			if (pTable->m_ItemType == 2 && pUser->m_lootSettings.iRare != 1)
				continue;
			if (pTable->m_ItemType == 1 && pUser->m_lootSettings.iMagic != 1)
				continue;
			if (pTable->m_ItemType == 4 && pUser->m_lootSettings.iUnique != 1)
				continue;
			if (pTable->m_bKind == 95 && pUser->m_lootSettings.iConsumable != 1)
				continue;
			if (pTable->m_iSellPrice < pUser->m_lootSettings.iPrice)
				continue;
	...
	bool bIsNeutralZone = (pUser->GetZoneID() == ZONE_DEATHMATCH || pUser->GetZoneID() == ZONE_KNIGHT_ROYALE || pUser->GetZoneID() == ZONE_CHAOS_DUNGEON || pUser->GetZoneID() == ZONE_DUNGEON_DEFENCE);
...
void CNpc::NoticeRespawn(std::string& Monster) {
	...
	if (!Monster.empty())
	{
		...
		sMonSummonNotice = string_format("The GM %s has summoned the %s in %s position(%d,%d).", g_pMain->NameGm.c_str(), Monster.c_str(), sZoneName.c_str(), uint16(GetX()), uint16(GetZ()));
