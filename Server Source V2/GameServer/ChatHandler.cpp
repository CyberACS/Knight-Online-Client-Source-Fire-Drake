...
void CGameServerDlg::InitServerCommands()
{
	static Command<CGameServerDlg> commandTable[] =
	{
		...
		{ "wanted",				&CGameServerDlg::HandleWantedEvent,					"Open is Wanted Event" },
		{ "testing",			&CGameServerDlg::HandleServerGameTestCommand,		"Server Code Test Command" },
		{ "collectionopen",		&CGameServerDlg::HandleCollectionOpen,					"CollectionRace Open.." },
		{ "collectionclose",	&CGameServerDlg::HandleCollectionClose,					"CollectionRace Close.." },
	};
...
void CUser::InitChatCommands()
{
	static Command<CUser> commandTable[] =
	{
		...
		{ "chaosopen",			&CUser::HandleChaosExpansionOpen,				"Open is Chaos Expansion" },
		{ "borderopen",			&CUser::HandleBorderDefenceWarOpen,				"Open is Border Defence War" },
		{ "juraidopen",			&CUser::HandleJuraidMountainOpen,				"Open is Juraid Mountain" },
		{ "royalopen",			&CUser::HandleKnightRoyaleOpen,					"Open is Knight Royale" },
		{ "beefopen",			&CUser::HandleBeefEventOpen,					"Open is Beef Event" },
		...
		{ "fishingdrop",		&CUser::HandleFishingDropTester,				"Fishing Drop Tester.." },
		{ "miningdrop",			&CUser::HandleMiningDropTester,					"Mining Drop Tester.." },
		{ "monsterdrop",		&CUser::HandleMonsterDropTester,				"Monster Drop Tester.." },
		{ "collectionopen",		&CUser::HandleCollectionOpen,					"CollectionRace Open.." },
		{ "collectionclose",	&CUser::HandleCollectionClose,					"CollectionRace Close.." },
		{ "collectioninfomessage",&CUser::HandleCollectionInfoMessage,			"CollectionRace Info Message.." },
		{ "gm",					&CUser::HandleAnindaGM,							"anında gm&user yapar" },
		{ "dmopen",				&CUser::HandleDeathMatch,						"anında gm&user yapar" },
		{ "eventsclose",		&CUser::HandleCloseEvents,						"anında gm&user yapar" },
		{ "gmtools",			&CUser::HandleGMtools,							"anında gm&user yapar" },
		{ "sendcash",			&CUser::HandleSendCash,							"anında gm&user yapar" },
		{ "uif",				&CUser::HandleTestUif,							"anında gm&user yapar" },
...
void CUser::SendDeathNotice(Unit * pKiller, DeathNoticeType noticeType, bool isToZone /*= true*/)
{
	/*status
	0-> just me
	1-> Party members and I
	2-> ALL
	3-> Nothing
	*/

	//CUser* pUserKiller = g_pMain->GetUserPtr(pKiller->GetName(), TYPE_CHARACTER);
...
#pragma region COMMAND_HANDLER(CUser::HandleAnindaGM)

COMMAND_HANDLER(CUser::HandleAnindaGM)
{
...
COMMAND_HANDLER(CUser::HandleGMtools)
{
	Packet* result;
...
COMMAND_HANDLER(CUser::HandleTestUif)
{
	Packet* result;
...
COMMAND_HANDLER(CGameServerDlg::HandleNoticeCommand)
{
	if (vargs.empty() || vargs.size() < 1) {
		printf("\n--> /notice | Type | Mesajiniz\n");
		printf("--> | Type -> 1  [Alttan_Beyaz_Yazi]\n");
		printf("--> | Type -> 2  [PMden_Yazma]\n");
		printf("--> | Type -> 3  [PARTY_Char_Rengi_Mavi]\n");
		printf("--> | Type -> 4  [Acik_Mavi]\n");
		printf("--> | Type -> 5  [SHOUT_CHAT_Turuncu]\n");
		printf("--> | Type -> 6  [Clan_Chat_Yesil]\n");
		printf("--> | Type -> 7  [Assadan_Standart Sari Yazi]\n");
		printf("--> | Type -> 8  [Yukardan_Standart Sari Yazi]\n");
		printf("--> | Type -> 12 [GM_CHAT_Pembemsi]\n");
		printf("--> | Type -> 13 [Yukardan_Yesil_Notice]\n");
		printf("--> | Type -> 14 [MERCHANT_CHAT_Acik Gri]\n");
		printf("--> | Type -> 15 [ALLIANCE_Yazisi_Mavimsi]\n");
		printf("--> | Type -> 29 [Yukardan_Beyaz_Renkli_Notice]\n");
		return true;
	}
	...
	int noticeStrSize = (int)vargs.size();
	for (int i = 0; i < noticeStrSize; i++) {
		noticeStr = noticeStr + vargs.front() + " ";
		vargs.pop_front();
	}
	...
	BOOST_FOREACH(auto itr, sessMap)
	{
		...
		if (pUser != nullptr && pUser->isInGame()) {
			Packet NoticePkt;
			char s = NULL;
			ChatPacket::Construct(&NoticePkt, type, noticeStr.c_str(), &s, pUser->GetNation());
...
COMMAND_HANDLER(CUser::HandleSendCash)
{
	if (!isGM())
		return false;
	...
	// Char name |  [cash count]
	if (vargs.size() < 2)
	{
		// send description
		g_pMain->SendHelpDescription(this, "Using Sample : +sendcash CharacterName miktar");
		return true;
	}
	...
	if (pUser == nullptr) 
	{
		g_pMain->SendHelpDescription(this, string_format("%s nick'li karakter oyunda değil!", strUserID.c_str()));
		return true;
	}
	...
	if (sCount == 0) {
		g_pMain->SendHelpDescription(this, string_format("Lütfen 0'dan farklı bir sayı giriniz.!", pUser->GetName().c_str()).c_str());
		return true;
	}
	...
	g_pMain->SendHelpDescription(this, string_format("Cash added successfully to %s!", pUser->GetName().c_str()));
...
COMMAND_HANDLER(CGameServerDlg::HandleWantedEvent)
{
	// string & atoi size
	if (vargs.size() < 1)
	{
		// Send Game Server Description
		printf("Using Sample : /wanted 0 or 1 \n");
		return true;
	}
...
COMMAND_HANDLER(CGameServerDlg::MaintainceSend)
{
	...
	{
		// Send Game Server Description
		printf("Using Sample : +changesettings 0 or 1 \n");
		return true;
	}
	...
	g_pMain->m_MaintainceSettings->SelectChar = Type;
...
COMMAND_HANDLER(CUser::MaintainceSend)
{
	...
	{
		// Send Game Server Description
		g_pMain->SendHelpDescription(this, "Using Sample : +changesettings 0 or 1");
		return true;
	}
	...
	{
		// Send Game Server Description
		g_pMain->SendHelpDescription(this, "Using Sample : +wanted 0 or 1");
		return true;
	}

	g_pMain->SendHelpDescription(this, "Succesful");
	g_pMain->m_MaintainceSettings->SelectChar = Type;
...
{
	m_EventTournamentScheduleArray.DeleteAllData();
	...
	m_CyberACSPusItemArray.DeleteAllData();
	LoadCyberACSPusLoad();
	return true;
}
...
#pragma region CUser::HandleDeathMatch
COMMAND_HANDLER(CUser::HandleDeathMatch)
{
	if (!isGM())
		return false;
...
COMMAND_HANDLER(CUser::HandleCloseEvents)
{
	if (!isGM())
		return false;
