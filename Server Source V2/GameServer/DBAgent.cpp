...
/*void CDBAgent::LoadSupportData(std::string& konu, std::string& icerik)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return;
...
void CDBAgent::LoadSlave(std::string& strAccountID)
{
	...
	CUser* pUser = g_pMain->GetUserPtr(strAccountID, TYPE_CHARACTER);
...
void CDBAgent::UpdateNationIntro(uint32 iServerNo, uint8 nation, std::string& strKingName, std::string& strKingNotice)
{
	...
	if (nation == 1)
	{
		if (!dbCommand->Execute(string_format(_T("UPDATE CYBERACS_SERVER_LIST SET strKarusKingName = ?, strKarusNotice = ? WHERE sGroupID = %d"), iServerNo)))
		{
			ReportSQLError(m_AccountDB->GetError());
			return;
		}
	...
	{
		if (!dbCommand->Execute(string_format(_T("UPDATE CYBERACS_SERVER_LIST SET strElMoradKingName = ?, strElMoradNotice = ? WHERE sGroupID = %d"), iServerNo)))
		{
			ReportSQLError(m_AccountDB->GetError());
			return;
		}
...
void CDBAgent::Support(std::string& subject, std::string& message)
{
	...
	if (!dbCommand->Execute(string_format(_T("{CALL SUPPORTS(?, ?)}"))))
		ReportSQLError(m_GameDB->GetError());
...
uint8 CDBAgent::LoadPPCard(std::string& PPCardKeys, std::string& strCharID)
{
	uint32 Cash = 0;
	if (!PPCardKeys.empty())
	{
		...
		if (!dbCommand->Execute(_T("SELECT * FROM PPCARD_LIST WHERE PPKeyCode = ?")))
			ReportSQLError(m_GameDB->GetError());

		if (dbCommand->hasData())
			dbCommand->FetchUInt32(2, Cash);

		if (Cash <= 0)
			return 0;

		CUser* pUser = g_pMain->GetUserPtr(strCharID, TYPE_CHARACTER);
		if (pUser == nullptr)
			return 0;

		dbCommand->AddParameter(SQL_PARAM_INPUT, PPCardKeys.c_str(), PPCardKeys.length());
		if (!dbCommand->Execute(_T("DELETE FROM PPCARD_LIST WHERE PPKeyCode = ?")))
			ReportSQLError(m_GameDB->GetError());

		pUser->SendKnightCash(Cash);
		return 1;
	}
	return 0;
}
...
#pragma region CDBAgent::LoadPasswd(std::string AccountID, std::string & strPasswd)
...
bool CDBAgent::LoadGameMasterSetting(string& strAccountID, string& strCharID, uint16 sServerNo, CUser* pUser)
{
	...
	dbCommand->FetchByte(field++, pUser->m_GameMastersGmTools);
...
bool CDBAgent::LoadServerSettings()
{
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_SERVER_SETTINGS}")))
	{
		ReportSQLError(m_GameDB->GetError());
	...
	do
	{
		_CYBERACS_SERVER_SETTINGS* pData = new _CYBERACS_SERVER_SETTINGS;
...
#pragma region CDBAgent::LoadCyberACSSettings()
bool CDBAgent::LoadCyberACSSettings()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_SETTINGS}")))
	{
		printf(" Error LOAD_CYBERACS_SETTINGS. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		uint32 id = 0;
		int i = 1;
		_CYBERACS_SETTINGS* pData = new _CYBERACS_SETTINGS;
		...
		dbCommand->FetchByte(i++, pData->Button_Youtube);
		dbCommand->FetchByte(i++, pData->Button_Twitter);
		dbCommand->FetchByte(i++, pData->Button_Skype);
		...
		dbCommand->FetchString(i++, pData->URL_Youtube);
		dbCommand->FetchString(i++, pData->URL_Twitter);
		dbCommand->FetchString(i++, pData->URL_Skype);
		...
		if (!g_pMain->m_CyberACSSettingsArray.PutData(id, pData))
			delete pData;
...
#pragma region CDBAgent::LoadCyberACSPusLoad()
bool CDBAgent::LoadCyberACSPusLoad()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_PUS}")))
	{
		printf(" Error LOAD_CYBERACS_PUS. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	{
		int i = 1;
		_CYBERACS_PUS_ITEM* pData = new _CYBERACS_PUS_ITEM;
		...
		if (!g_pMain->m_CyberACSPusItemArray.PutData(pData->sItemID, pData))
			delete pData;
...
bool CDBAgent::MannerStoreLoad()
{
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_MANNER_STORE}")))
	{
		printf(" Error LOAD_MANNER_STORE. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	{
		int i = 1;
		_CYBERACS_MANNER_STORE_ITEM* pData = new _CYBERACS_MANNER_STORE_ITEM;
		...
		if (!g_pMain->m_MannerStoreArray.PutData(pData->sItemID, pData))
			delete pData;
...
#pragma region CDBAgent::LoadCyberACSCollectionRaceSettings()
bool CDBAgent::LoadCyberACSCollectionRaceSettings()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_COLLECTIONRACE_SETTINGS}")))
	{
		printf(" Error LOAD_CYBERACS_COLLECTIONRACE_SETTINGS. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_CYBERACS_COLLECTIONRACE_SETTINGS* pData = new _CYBERACS_COLLECTIONRACE_SETTINGS;
		...
		if (!g_pMain->m_CyberACSCollectionRaceSettingsArray.PutData(pData->ID, pData))
			delete pData;
...
#pragma region CDBAgent::LoadCyberACSCollectionRaceSettings()
bool CDBAgent::LoadSearchItem(std::string& ItemName)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());

	if (dbCommand.get() == nullptr)
		return false;
	dbCommand->AddParameter(SQL_PARAM_INPUT, ItemName.c_str(), ItemName.length());
	if (!dbCommand->Execute(_T("{CALL SEARCH_ITEM_MONSTER(?)}")))
	{
		printf(" Error SEARCH_ITEM_MONSTER. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_MONSTER_SEARCH_ITEM* pData = new _MONSTER_SEARCH_ITEM;

		int i = 1;

		dbCommand->FetchUInt32(i++, pData->ID);

		dbCommand->FetchString(i++, pData->Name);
		dbCommand->FetchString(i++, pData->ZoneName);
		dbCommand->FetchUInt16(i++, pData->ZoneNo);
		dbCommand->FetchUInt16(i++, pData->LeftX);
		dbCommand->FetchUInt16(i++, pData->RightX);

		if (!g_pMain->m_SearchItemArrays.PutData(pData->ID, pData))
			delete pData;
...
#pragma region CDBAgent::LoadCyberACSCollectionRaceSettings()
bool CDBAgent::LoadLotterySettings()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_LOTTERY_SETTINGS}")))
	{
		printf(" Error LOAD_CYBERACS_LOTTERY_SETTINGS. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_CYBERACS_LOTTERY_SETTINGS* pData = new _CYBERACS_LOTTERY_SETTINGS;
		...
		for (int j = 0; j < 3; j++)
			dbCommand->FetchUInt32(i++, pData->NumReward[j]);

		for (int j = 0; j < 3; j++)
			dbCommand->FetchUInt32(i++, pData->EventItemCount[j]);

		for (int j = 0; j < 3; j++)
			dbCommand->FetchUInt32(i++, pData->EventItemTimed[j]);


		if (!g_pMain->m_LotteryArray.PutData(pData->ID, pData))
			delete pData;
...
#pragma region CDBAgent::LoadCyberACSCollectionRaceHuntList()
bool CDBAgent::LoadCyberACSCollectionRaceHuntList()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL LOAD_CYBERACS_COLLECTIONRACE_HUNT_LIST}")))
	{
		printf(" Error LOAD_CYBERACS_COLLECTIONRACE_HUNT_LIST. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_CYBERACS_COLLECTIONRACE_HUNT_LIST* pData = new _CYBERACS_COLLECTIONRACE_HUNT_LIST;
		...
		if (!g_pMain->m_CyberACSCollectionRaceHuntListArray.PutData(pData->QuestID, pData))
			delete pData;
...
bool CDBAgent::UpdateDailyQuest(uint32 m_sSelectMonster, uint8 Open)
{
	...
	if (!dbCommand->Execute(string_format(_T("UPDATE CYBERACS_DAILY_HUNT_LIST SET isOpen = %d WHERE ID = %d"), Open, m_sSelectMonster)))
		ReportSQLError(m_AccountDB->GetError());
}


bool CDBAgent::UpdateDailyQuestUser(uint8 Quest, std::string& strAccountID)
{
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strAccountID.c_str(), strAccountID.length());
	if (!dbCommand->Execute(string_format(_T("UPDATE CYBERACS_DAILY_USER SET Quest%d = 1 WHERE strUser = ?"), Quest)))
		ReportSQLError(m_AccountDB->GetError());

	return true;
}

bool CDBAgent::ClearDailyQuestUser()
{
	...
	if (!dbCommand->Execute(_T("{CALL CLEAR_DAILY_USER}")))
	{
		printf(" Error [CLEAR_DAILY_USER]. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
...
bool CDBAgent::LoadUserQuests(std::string& strAccountID)
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	if (dbCommand.get() == nullptr)
		return false;
	...
	dbCommand->AddParameter(SQL_PARAM_INPUT, strAccountID.c_str(), strAccountID.length());
	if (!dbCommand->Execute(_T("{CALL LOAD_USER_QUESTS(?)}")))
	{
		printf(" Error [LOAD_USER_QUESTS]. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		dbCommand->FetchUInt32(1, Quest[0]);
		dbCommand->FetchUInt32(2, Quest[1]);
		dbCommand->FetchUInt32(3, Quest[2]);
		dbCommand->FetchUInt32(4, Quest[3]);
		dbCommand->FetchUInt32(5, Quest[4]);
		dbCommand->FetchUInt32(6, Quest[5]);
		dbCommand->FetchUInt32(7, Quest[6]);
		dbCommand->FetchUInt32(8, Quest[7]);
		dbCommand->FetchUInt32(9, Quest[8]);
		dbCommand->FetchUInt32(10, Quest[9]);
		dbCommand->FetchUInt32(11, Quest[10]);
		dbCommand->FetchUInt32(12, Quest[11]);
		dbCommand->FetchUInt32(13, Quest[12]);
		dbCommand->FetchUInt32(14, Quest[13]);
		dbCommand->FetchUInt32(15, Quest[14]);
		dbCommand->FetchUInt32(16, Quest[15]);
		dbCommand->FetchUInt32(17, Quest[16]);
		dbCommand->FetchUInt32(18, Quest[17]);
		dbCommand->FetchUInt32(19, Quest[18]);
		dbCommand->FetchUInt32(20, Quest[19]);
		dbCommand->FetchUInt32(21, Quest[20]);
		dbCommand->FetchUInt32(22, Quest[21]);
		dbCommand->FetchUInt32(23, Quest[22]);
		dbCommand->FetchUInt32(24, Quest[23]);
		dbCommand->FetchUInt32(25, Quest[24]);
		dbCommand->FetchUInt32(26, Quest[25]);
		dbCommand->FetchUInt32(27, Quest[26]);
		dbCommand->FetchUInt32(28, Quest[27]);
		dbCommand->FetchUInt32(29, Quest[28]);
		dbCommand->FetchUInt32(30, Quest[29]);

	} while (dbCommand->MoveNext());

	for (int i = 0; i < 30; i++) {
		if (Quest[i] == 1)
			pUser->m_DailyRegister[i] = false;
		else if (Quest[i] == 0)
			pUser->m_DailyRegister[i] = true;
	}
...
bool CDBAgent::LoadDailyHuntList()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL [LOAD_CYBERACS_DAILY_HUNT_LIST]}")))
	{
		printf(" Error [LOAD_CYBERACS_DAILY_HUNT_LIST]. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_CYBERACS_DAILY_HUNT_LIST* pData = new _CYBERACS_DAILY_HUNT_LIST;

		int i = 1;

		dbCommand->FetchByte(i++, pData->QuestID);

		dbCommand->FetchString(i++, pData->QuestName);
		dbCommand->FetchUInt16(i++, pData->isOpen);
		dbCommand->FetchByte(i++, pData->SettingsID);
		...
		if (!g_pMain->m_DailyHuntListArray.PutData(pData->QuestID, pData))
			delete pData;
...
bool CDBAgent::LoadDailySettings()
{
	unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
	...
	if (!dbCommand->Execute(_T("{CALL [LOAD_CYBERACS_DAILY_SETTINGS]}")))
	{
		printf(" Error [LOAD_CYBERACS_DAILY_SETTINGS]. \n");
		ReportSQLError(m_GameDB->GetError());
		return false;
	}
	...
	do
	{
		_CYBERACS_DAILY_SETTINGS* pData = new _CYBERACS_DAILY_SETTINGS;
		...
		if (!g_pMain->m_DailySettingsArray.PutData(pData->ID, pData))
			delete pData;
...
#pragma region bool CDBAgent::LoadLootSettings(std::string& strCharID, CUser* pUser)

void CDBAgent::LoadLootSettings(std::string& strCharID, CUser* pUser)
{
	if (!strCharID.empty())
	{
		if (pUser->GetName() == strCharID)
		{
			unique_ptr<OdbcCommand> dbCommand(m_GameDB->CreateCommand());
			if (dbCommand.get() == nullptr)
				return;
			...
			if (!dbCommand->Execute(_T("SELECT iWarrior, iRogue, iMage, iPriest, iWeapon, iArmor, iAccessory, iNormal, iUpgrade, iCraft, iRare, iMagic, iUnique, iConsumable, iPrice FROM CYBERACS_USER_LOOT_SETTINGS WHERE UserID = ?")))
				ReportSQLError(m_GameDB->GetError());

			if (dbCommand->hasData())
			{
				int i = 1;
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iWarrior);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iRogue);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iMage);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iPriest);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iWeapon);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iArmor);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iAccessory);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iUpgrade);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iCraft);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iRare);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iMagic);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iUnique);
				dbCommand->FetchByte(i++, pUser->m_lootSettings.iConsumable);
				dbCommand->FetchUInt32(i++, pUser->m_lootSettings.iPrice);
			}
		}
	}
}
...
#pragma region bool UpdateLootSettings(CUser* pUser, std::string& strCharID, uint8_t iWarrior, uint8_t iRogue, uint8_t iMage, uint8_t iPriest, uint8_t iWeapon, uint8_t iArmor, uint8_t iAccessory, uint8_t iNormal, uint8_t iUpgrade, uint8_t iCraft, uint8_t  iRare, uint8_t iMagic, uint8_t iUnique, uint8_t iConsumable, uint32_t iPrice) 

bool CDBAgent::UpdateLootSettings(CUser* pUser, std::string& strCharID, uint8_t iWarrior, uint8_t iRogue, uint8_t iMage, uint8_t iPriest, uint8_t iWeapon, uint8_t iArmor, uint8_t iAccessory, uint8_t iNormal, uint8_t iUpgrade, uint8_t iCraft, uint8_t  iRare, uint8_t iMagic, uint8_t iUnique, uint8_t iConsumable, uint32_t iPrice)
{

	if (strCharID != pUser->GetName())
		return false;

	pUser->m_lootSettings.iWarrior = iWarrior;
	pUser->m_lootSettings.iRogue = iRogue;
	pUser->m_lootSettings.iMage = iMage;
	pUser->m_lootSettings.iPriest = iPriest;
	pUser->m_lootSettings.iWeapon = iWeapon;
	pUser->m_lootSettings.iArmor = iArmor;
	pUser->m_lootSettings.iAccessory = iAccessory;
	pUser->m_lootSettings.iNormal = iNormal;
	pUser->m_lootSettings.iUpgrade = iUpgrade;
	pUser->m_lootSettings.iCraft = iCraft;
	pUser->m_lootSettings.iRare = iRare;
	pUser->m_lootSettings.iMagic = iMagic;
	pUser->m_lootSettings.iUnique = iUnique;
	pUser->m_lootSettings.iConsumable = iConsumable;
	pUser->m_lootSettings.iPrice = iPrice;
	...
	if (!dbCommand->Execute(string_format(_T("UPDATE CYBERACS_USER_LOOT_SETTINGS SET iWarrior=%d, iRogue=%d, iMage=%d, iPriest=%d, iWeapon=%d, iArmor=%d, iAccessory=%d, iNormal=%d, iUpgrade=%d, iCraft=%d, iRare=%d, iMagic=%d, iUnique=%d, iConsumable=%d, iPrice=%d WHERE UserID = ?"), iWarrior, iRogue, iMage, iPriest, iWeapon, iArmor, iAccessory, iNormal, iUpgrade, iCraft, iRare, iMagic, iUnique, iConsumable, iPrice)))
		ReportSQLError(m_GameDB->GetError());

	return true;
}

void CDBAgent::UpdateVSEventLog(std::string WinNames, std::string LoseNames, uint32 NpPoint)
{
	...
	if (!dbCommand->Execute(string_format(_T("INSERT INTO CYBERACS_VS_EVENT_LOGS(WinnerPlayer, LoserPlayer, NationalPoint) VALUES('%s', '%s', '%d')"), WinNames.c_str(), LoseNames.c_str(), NpPoint)))
	{
		ReportSQLError(m_GameDB->GetError());
		printf("[Vs Event Log Manager] = Database Insert Log Error\n");
		return;
	}
