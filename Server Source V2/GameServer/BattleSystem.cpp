...
void CGameServerDlg::BattleZoneRemnantSpawn()
{
	foreach_stlmap(itr, m_WarBanishOfWinnerArray)
	{
		...
		SpawnEventNpc(itr->second->sSid, true, itr->second->sZoneID, itr->second->sPosX, 0, itr->second->sPosZ, itr->second->sCount, itr->second->sRadius, itr->second->sDeadTime * 60, 0, -1, 0, 0, 1, 0, 0, 0);
	}
