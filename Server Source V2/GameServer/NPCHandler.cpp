...
void CUser::ClassChange(Packet & pkt, bool bFromClient /*= true */)
{
	...
	{
		if (m_bSkillStatResetHook == 1)
			AllPointChange(false);
		else if (m_bSkillStatResetHook == 2)
			AllSkillPointChange(false);
		else if (m_bSkillStatResetHook == 0)
			AllSkillPointChange(false);
		return;
	}
	else if (opcode == CHANGE_MONEY_REQ)
	{
		...
		pkt >> sub_type >> m_bSkillStatResetHook;
		if (sub_type == 1)
		{
			...
			else if(m_bSkillStatResetHook > 0)
			{
				...
				if (MoneyResult == 0)
					return;
				result << uint8(CHANGE_MONEY_REQ) << MoneyResult;
				Send(&result);
				return;
			}
			else
