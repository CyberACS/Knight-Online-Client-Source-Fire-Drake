...
bool CGameServerDlg::Startup()
{
	...
	SelectChar = g_DBAgent.GAME_SERVER_SETTINGS();
	
	// License System Stard System..
	/*m_HardwareIDArray.push_back(16813151825824);
	m_sHardwareIpArray.push_back("51.195.76.207");
	...
	SetMiningDrops();
	UpdateCyberACSCollectionRaceInfoTime();
	UpdateDailyInfoTime();

	printf("\nCyberACS Best MMORPG Game Developer System..\n");
	printf("\nwww.cyberacs.com\n");
	return true;
}
...
bool CGameServerDlg::LoadTables()
{
	if (!GameStartClearRemainUser()
		...
		|| !LoadTournamentScheduleStatusTable()
		...
		|| !LoadBotTable()
		|| !LoadCyberACSSettings()
		|| !LoadCyberACSPusLoad()
		|| !MannerStoreLoad()
		|| !LoadCyberACSCollectionRaceSettings()
		|| !GAME_SERVER_SETTINGS()
		|| !LoadLotterySettings()
		|| !LoadCyberACSCollectionRaceHuntList()
		|| !LoadDailySettings()
		|| !LoadDailyHuntList()
		|| !LoadNpcPosTable()
		|| !CreateNpcThread()
		|| !LoadDeathMatchTable()
		|| !LoadDeathMatchTable1()
		|| !LoadDeathMatchTable2()
		|| !LoadDeathMatchTable3()
		|| !LoadMonsterBowlTable() // Bowl Event MeLanor
		|| !ChaosStoneLoad())
		return false;

	foreach_stlmap(itr, m_CyberACSSettingsArray)
		m_CyberACSSettings = itr->second;

	if (m_CyberACSSettings == nullptr)
	{
		printf("CyberACS Settings Tables Nullptr \n");
		return false;
	}

	m_CyberACSCollectionRaceSetting = m_CyberACSCollectionRaceSettingsArray.GetData(m_nServerNo);
	if (m_CyberACSCollectionRaceSetting == nullptr)
	{
		printf("CyberACS CollectionRace Settings Tables Nullptr \n");
		return false;
	}

	m_CyberACSCollectionRaceHuntList = m_CyberACSCollectionRaceHuntListArray.GetData(m_nServerNo);
	if (m_CyberACSCollectionRaceHuntList == nullptr)
	{
		printf("CyberACS CollectionRace Hunt List Tables Nullptr \n");
		return false;
	}
	return true;
}
...
void CGameServerDlg::GetEventAwardsIni()
{
	...
	BownEvent = ini.GetBool("BOWL_X_EVENT", "status", 1); // Bowl Event MeLanor

	m_DeathMatchItem1 = ini.GetBool("DeathMatchItem", "Item01", BLUE_TREASURE_CHEST);
	m_DeathMatchItem2 = ini.GetBool("DeathMatchItem", "Item02", 1);
	m_DeathMatchItem3 = ini.GetBool("DeathMatchItem", "Item03", 1);
}
/**
...
void CGameServerDlg::GetTimeFromIni()
{
	...
	MaxLevel62Control = ini.GetInt("GAME", "MAXLV62CONTROL", 0); //Ardream Event MeLanor
	...
	m_sCapeAC = ini.GetInt("SUPERIOR_CAPE_BONUS", "DEFANCE_BONUS", 0);
	m_sCapeAtk = ini.GetInt("SUPERIOR_CAPE_BONUS", "ATTACK_BONUS", 0);
	m_sCapeHp = ini.GetInt("SUPERIOR_CAPE_BONUS", "HEALTH_BONUS", 0);
	m_sCapeNp = ini.GetInt("SUPERIOR_CAPE_BONUS", "LOYALTY_BONUS", 0);

	m_sJackExpPots = ini.GetInt("JACK_POTS", "JACK_EXP_POT", 0);
	m_sJackGoldPots = ini.GetInt("JACK_POTS", "JACK_NOAH_POT", 0);
	PetFree = ini.GetBool("PET_FREE", "STATUS", true);
	...
	for (int i = 71; i <= 73; i++)
	{
		_WANTED_MAIN_CONTROL* pMain = new _WANTED_MAIN_CONTROL();
		pMain->Initialize();
		pMain->EventZoneID = i;
		m_WantedEventMainControlArray.PutData(i, pMain);
	}
	...
	//CollectionRace
	m_CyberACSCollectionRaceCounts = 0;
	m_CyberACSCollectionRaceFinishTime = 0;
	m_CyberACSCollectionRaceGameServerSatus = false;
	memset(m_CyberACSCollectionRaceQuestID, 0, sizeof(m_CyberACSCollectionRaceQuestID));
	memset(m_CyberACSCollectionRaceQuestZone, 0, sizeof(m_CyberACSCollectionRaceQuestZone));
	memset(m_CyberACSCollectionRaceMonsterNum1, 0, sizeof(m_CyberACSCollectionRaceMonsterNum1));
	memset(m_CyberACSCollectionRaceMonsterNum2, 0, sizeof(m_CyberACSCollectionRaceMonsterNum2));
	memset(m_CyberACSCollectionRaceMonsterNum3, 0, sizeof(m_CyberACSCollectionRaceMonsterNum3));
	memset(m_CyberACSCollectionRaceMonsterKillCount1, 0, sizeof(m_CyberACSCollectionRaceMonsterKillCount1));
	memset(m_CyberACSCollectionRaceMonsterKillCount2, 0, sizeof(m_CyberACSCollectionRaceMonsterKillCount2));
	memset(m_CyberACSCollectionRaceMonsterKillCount3, 0, sizeof(m_CyberACSCollectionRaceMonsterKillCount3));


	for (int i = 0; i < MAX_CR_EVENT_COUNT; i++)
	{
		//DailyQuest
		m_DailyCounts = 0;
		m_DailyFinishTime[i] = 0;
		m_DailyGameServerSatus[i] = false;
		memset(m_DailyQuestID, 0, sizeof(m_DailyQuestID[i]));
		memset(m_DailyQuestZone, 0, sizeof(m_DailyQuestZone[i]));
		memset(m_DailyMonsterNum1, 0, sizeof(m_DailyMonsterNum1[i]));
		memset(m_DailyMonsterNum2, 0, sizeof(m_DailyMonsterNum2[i]));
		memset(m_DailyMonsterNum3, 0, sizeof(m_DailyMonsterNum3[i]));
		memset(m_DailyMonsterNum4, 0, sizeof(m_DailyMonsterNum4[i]));
		memset(m_DailyMonsterKillCount1, 0, sizeof(m_DailyMonsterKillCount1[i]));
		memset(m_DailyMonsterKillCount2, 0, sizeof(m_DailyMonsterKillCount2[i]));
		memset(m_DailyMonsterKillCount3, 0, sizeof(m_DailyMonsterKillCount3[i]));
		memset(m_DailyMonsterKillCount4, 0, sizeof(m_DailyMonsterKillCount4[i]));
	}
...
Unit* CGameServerDlg::GetUnitPtr(uint16 id, uint16 sZoneID /*= 0*/)
{
	...
	pPriest = g_pMain->m_BotArray.GetData(id);

	if(pPriest != nullptr && !pPriest->isNPC() )
	return GetBotPtr(pPriest->GetName(), TYPE_CHARACTER);
...
uint32 CGameServerDlg::Timer_CheckGameEvents(void* lpParam)
{
	while (g_bRunning)
	{
		...
		g_pMain->CyberACSCollectionRaceFinish();
		g_pMain->DailyFinish();
...
void CGameServerDlg::AutoOnlineCount() //yenicount
{
	...
	if ((nMinute % 15 == 0 || nMinute == 0) && nSecond == 0)
	{
		...
		std::string sNoticeMessage = string_format("Online User Count : [%d] Thank you for being with us.", count); //bilgiyi oyundaki tüm kullanıcılara gönderir
