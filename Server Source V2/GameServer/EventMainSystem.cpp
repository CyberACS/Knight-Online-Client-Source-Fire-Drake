...
void CGameServerDlg::TempleEventScreen(uint8 ActiveEvent, uint16 sTime)
{
	...
	else if (ActiveEvent == DEATHMATCH)
		aresult << uint32(9) << uint8(24) << uint32(sTime);/* Time */
...
void CGameServerDlg::TempleEventSendWinnerScreen()
{
	...
	case DEATHMATCH:
	{
		for (int i = 1; i <= MAX_TEMPLE_EVENT_ROOM; i++)
		{
			...
			Packet pkt1(WIZ_EVENT);
			pkt1 << uint8(TEMPLE_EVENT_FINISH) << uint16(TEMPLE_EVENT_CHAOS) << uint8(104) << uint16(20) << uint16(0);
			Send_All(&pkt1, nullptr, Nation::ALL, ZONE_DEATHMATCH, true, i);
		}
		TempleEventChaosShowCounterScreen();
	}
	break;
...
void CGameServerDlg::TempleEventStart()
{
	if (pTempleEvent.ActiveEvent == TEMPLE_EVENT_JURAD_MOUNTAIN)
	{
		...
		foreach(itr, sessMap)
		{
			...
			Packet result1(PL_EVENT, uint8(TEMPLE_EVENT));
			result1 << (uint16)pTempleEvent.ActiveEvent << m_nVirtualEventRoomRemainSeconds;
			pUser->Send(&result1);
		...
		|| pTempleEvent.ActiveEvent == TEMPLE_EVENT_CHAOS)
	{
		...
		foreach(itr, sessMap)
		{
			...
			Packet result(PL_EVENT, uint8(TEMPLE_EVENT));
			result << (uint16)pTempleEvent.ActiveEvent << m_nVirtualEventRoomRemainSeconds;
			pUser->Send(&result);
		}
	}

	if (pTempleEvent.ActiveEvent == DEATHMATCH)
	{
		SessionMap sessMap = g_pMain->m_socketMgr.GetActiveSessionMap();
		foreach(itr, sessMap)
		{
...
void CUser::EventPartyInvitationCheck(uint16 EnterPartyGetID)
{
	...
	if (pEnterPartyUser == nullptr
		...
		|| pEnterPartyUser->ÝsInApprovedParty()
	...
	case DEATHMATCH:
	{
		foreach_stlmap(itr, g_pMain->m_TempleEventChaosRoomList)
		{
			...
			foreach_stlmap_nolock(itrUser, pRoomInfo->m_UserList)
			{
				...
				pUser->m_bEventRoom = itr->first;
				//pUser->ChaosExpansionAddPlayerRank();
				pUser->ZoneChange(pTempleEvent.ZoneID, float(128 + myrand(-20, +20)), float(128 + myrand(-20, +20)));
			}
			pRoomInfo->m_UserList.m_lock.unlock();
		}
	}
		...
		if (nRoom != -1)
		{
			...
			foreach_stlmap(itrUser, pRoomInfo->m_UserList)
			{
				...
				if (pUser == nullptr
					|| !pUser->isInGame()
					|| !pUser->isEventUser()
					|| pUser->GetZoneID() != ZONE_DEATHMATCH)
					continue;
		...
		else
		{
			foreach_stlmap(itr, g_pMain->m_TempleEventChaosRoomList)
			{
				...
				foreach_stlmap_nolock(itrUser, pRoomInfo->m_UserList)
				{
					...
					{
						pUser->AchieveWarCountAdd(UserAchieveWarTypes::AchieveWinChaos1, 0, nullptr);
						pUser->GiveItem(g_pMain->m_DeathMatchItem1, 1);
						pUser->GiveItem(g_pMain->m_DeathMatchItem2, 1);
						pUser->GiveItem(g_pMain->m_DeathMatchItem3, 1);
					}
...
void CGameServerDlg::TempleEventSendActiveEventTime(CUser* pUser)
{
	...
	case TEMPLE_EVENT_JURAD_MOUNTAIN:
		...
		break;
	case DEATHMATCH:
		TemplEventDeathMatchSendJoinScreenUpdate(pUser);
		break;
	default:
		break;
	}
...
void CGameServerDlg::TempleEventKickOutUser(CUser* pUser)
{
	...
	else if (pUser->GetZoneID() == ZONE_FORGOTTEN_TEMPLE)
	{
		if (pUser->GetLevel() < 35)
			nZoneID = ZONE_MORADON;
		else if (pUser->GetLevel() >= 35 && pUser->GetLevel() <= 59)
	...
	else if (pUser->GetZoneID() == ZONE_DEATHMATCH)
	{
		Packet result1(PL_EVENT);
		result1 << uint8(18);
		pUser->Send(&result1);

		if (pUser->GetLevel() < 35 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_MORADON;
		else if (pUser->GetLevel() >= 35 && pUser->GetLevel() <= 59 && pUser->GetNation() == 1 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_KARUS;
		else if (pUser->GetLevel() >= 35 && pUser->GetLevel() <= 59 && pUser->GetNation() == 2 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_ELMORAD;
		else if (pUser->GetLevel() >= 60 && pUser->GetLevel() <= 69 && pUser->GetNation() == 1 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_KARUS_ESLANT;
		else if (pUser->GetLevel() >= 60 && pUser->GetLevel() <= 69 && pUser->GetNation() == 2 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_ELMORAD_ESLANT;
		else if (pUser->GetLevel() >= 70 && pUser->GetLoyalty() > 0 && pUser->GetNation() == 1 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_KARUS_ESLANT;
		else if (pUser->GetLevel() >= 70 && pUser->GetLoyalty() > 0 && pUser->GetNation() == 2 && pUser->GetLoyalty() <= 0)
			nZoneID = ZONE_ELMORAD_ESLANT;
		else if (pUser->GetLevel() >= 70 && pUser->GetLoyalty() > 0)
			nZoneID = ZONE_RONARK_LAND;
...
void CGameServerDlg::TempleEventReset(uint16 bActiveEvent)
{
	...
	else if (bActiveEvent == DEATHMATCH)
	{
		RemoveAllEventNpc(ZONE_DEATHMATCH);
		foreach_stlmap(itr, m_TempleEventChaosRoomList)
		{
			if (itr->second == nullptr)
				continue;
...
void CGameServerDlg::DeathMatchPrepare()
{
	...
	foreach_stlmap(itr, m_TempleEventChaosRoomList)
	{
		...
		foreach_stlmap_nolock(itrUser, pRoomInfo->m_UserList)
		{
			...
			Packet result1(PL_EVENT);
			result1 << uint8(17) << uint16(18) << pPlayInfo->EventPlayTime * MINUTE;
			pUser->Send(&result1);

			C3DMap* pMap = g_pMain->GetZoneByID(ZONE_DEATHMATCH);
			if (pMap)
				pMap->UpdateDeathMatch(true, ZoneAbilitySiege1);
		...
		/*for (int i = 1; i <= MAX_TEMPLE_EVENT_ROOM; i++)
		{
			...
			Packet pkt1(PL_TIMER);
			pkt1 << uint8(17) << uint16(18) << pPlayInfo->EventPlayTime * MINUTE;
			Send_All(&pkt1, nullptr, Nation::ALL, ZONE_DEATHMATCH, true, i);
		}*/
		pRoomInfo->m_UserList.m_lock.unlock();
