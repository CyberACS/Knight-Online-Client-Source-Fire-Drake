...
void CUser::HpChange(int amount, Unit *pAttacker /*= nullptr*/, bool isDOT /*= false*/)
{
	...
	if (pAttacker)
	{
		CBot* pUserBot = g_pMain->m_BotArray.GetData(pAttacker->GetID());

		// No cheats allowed
		if (pUserBot != nullptr )//bot ise
		{
			if (pUserBot == nullptr && !pAttacker->isNPC())
				return;
		}
		else if (pAttacker->GetZoneID() != GetZoneID())
			return;
	}
		...
		if (pKiller != this)
		{
			...
			case ZONE_RONARK_LAND_BASE:
				if (pKiller->isInPKZone())
				{
					...
					{
						WantedEventDead();
						pKiller->WantedEventKilled();
					}

					if (g_pMain->m_CyberACSCollectionRaceGameServerSatus)
						pKiller->CyberACSCollectionRaceProcess(1);
				}
				break;
			case ZONE_ELMORAD:
			case ZONE_KARUS:
...
void CBot::HpChange(int amount, Unit* pAttacker /*= nullptr*/, bool isDOT /*= false*/)
{
	...
	if (amount < 0)
	{
		...
		if (m_bMirrorDamage && !NotUseZone)
		{
			if (m_bMirrorDamageType)
			{
				CBot* pUserAttacker = g_pMain->GetBotPtr(pAttacker->GetName(),TYPE_CHARACTER);
				if (pUserAttacker != nullptr)
				{
					mirrorDamage = (m_byMirrorAmount * amount) / 100;
					amount -= mirrorDamage;
