...
void CUser::MerchantOpen()
{
	...
	if (isSlaveMerchant())
	{
		...
		result.clear();
		result.Initialize(PL_INFOMESSAGE);
...
void CUser::MerchantItemAdd(Packet& pkt)// kc ekli deðil mi ? ekli
{
	return;
...
#pragma region SlaveMerchant
	if (SlaveTotalTime > 0 && isSlave == 1)
	{
		...
		if (m_bSlaveUsers > 0)
		{
			...
			for (int i = 0; i < MAX_MERCH_ITEMS; i++)
			{
				RobItem(pSlaveUser->m_arMerchantItems[i].nNum, pSlaveUser->m_arMerchantItems[i].sCount);
			}
...
void CUser::MerchantItemList(Packet& pkt)
{
	...
	if (pUserBot != nullptr)
	{
		...
		if (pUserBot->m_bMerchantViewer >= 0)
		{
			if (pUserBot->m_bMerchantOpen == true)
			{
				...
				DateTime time;
				/*Packet info(PL_MERC_WIEWER_INFO);
				info << uint8(1) << pUser->GetName() << time.GetHour() << time.GetMinute();
				Send(&info);*/
		...
		DateTime time;
		/*Packet info(PL_MERC_WIEWER_INFO);
		info << uint8(1) << pUserBot->GetName() << time.GetHour() << time.GetMinute();
		Send(&info);*/
		...
		{
			_MERCH_DATA* pMerch = &pUserBot->m_arMerchantItems[i];
			uint8 isKC = pMerch->isKC ? 1 : 0;//burasý deðil diðeri itemad
			result << isKC;
		}
	...
	string name = pUserMerchant->m_strUserID;
	DateTime time;
		...
		if (pUserMerchant->m_bMerchantOpen == true)
		{
			...
			if (pUser != nullptr)
			{
				...
				else
				{
					DateTime time;
					Packet info(PL_MERC_WIEWER_INFO);
					info << uint8(1) << pUserMerchant->GetName() << time.GetHour() << time.GetMinute();
					Send(&info);
...
void CUser::MerchantItemBuy(Packet& pkt)
{
	...
	if (pUserBot != nullptr)
	{
		...
		if (pMerch->isKC)
		{
			...
			{
				for (int i = 0; i < 12; i++) {
					if (SlaveItemID[i] == itemid)
					{
						SlaveItemID[i] = 0;
						SlaveItemCount[i] -= item_count;
				...
				if (pSlaveUser != nullptr) {
					...
					g_DBAgent.UpdateSlaveKC(GetName(), req_gold);
		...
		else
		{
			...
			if (pUserBot->isSlaveMerchant())
			{
				...
				if (pSlaveUser != nullptr)
				{
					...
					g_DBAgent.UpdateSlaveCoins(GetName(), req_gold);
	...
	_ITEM_DATA* pItem = GetItem(SLOT_MAX + dest_slot);
	_ITEM_DATA* pItemRob = &pMerchant->m_sItemArray[pMerch->bOriginalSlot]; //merchant reloads bug deneme
	if (pItem == nullptr)
	{
		result << uint8(6) << uint16(-18);
		Send(&result);
	...
	if (pMerch->isKC)
	{
		...
		/*if (pUserBot->isSlaveMerchant())
		{
			CUser* pSlaveUser = g_pMain->GetUserPtr(pUserBot->m_bSlaveUserID);
			if (pSlaveUser != nullptr)
				pSlaveUser->CyberACS_CashGain(req_gold);
		}*/
...
void CUser::CancelMerchant()
{
	...
	/*CUser* pMerchant = g_pMain->GetUserPtr(m_sMerchantsSocketID);
	CUser* pUser = g_pMain->GetUserPtr(pMerchant->m_bMerchantViewer);

	string name = pUser->m_strUserID;
	uint8 saat = time.GetHour();
	uint8 dakika = time.GetMinute();*/
	...
	/*result.Initialize(PL_MERC_WIEWER_INFO);
	result << uint8(1) << name << saat << dakika;
	Send(&result);*/
