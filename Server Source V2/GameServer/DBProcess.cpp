...
bool CDBProcess::Connect(string& szDSN, string& szUser, string& szPass)
{
	...
	{
		g_pLoginMain->ReportSQLError(m_dbConnection.GetError());
		return false;
	}
...
bool CDBProcess::LoadVersionList()
{
	...
	if (dbCommand->hasData())
	{
		...
		do
		{
			...
			g_pLoginMain->m_VersionList.insert(make_pair(pVersion->strFilename, pVersion));

			if (g_pLoginMain->m_sLastVersion < pVersion->sVersion)
				g_pLoginMain->m_sLastVersion = pVersion->sVersion;
...
bool CDBProcess::LoadServerList()
{
	...
	if (!dbCommand->Execute(_T("SELECT * FROM CYBERACS_SERVER_LIST")))
	{
		g_pLoginMain->ReportSQLError(m_dbConnection.GetError());
		return false;
	}

	if (g_pLoginMain->m_ServerList.GetSize() > 0)
		g_pLoginMain->m_ServerList.DeleteAllData();
	...
	if (dbCommand->hasData())
	{
		...
		do
		{
			...
			dbCommand->FetchInt16(i++, pInfo->SelectChar);
			...
			g_pLoginMain->m_ServerList.PutData(pInfo->sServerID, pInfo);
			nServerCount++;
...
bool CDBProcess::OTP(string& strAccountID, uint32 strPasswd)
{
	...
	oSmtp->UserName = _T("CyberACS@gmail.com");
...
bool CDBProcess::LoadUserCountList()
{
	...
	if (dbCommand->hasData())
	{
		do
		{
			...
			_SERVER_INFO* pInfo = g_pLoginMain->m_ServerList.GetData(serverID);
			if (pInfo)
...
uint16 CDBProcess::AccountLogin(string& strAccountID, string& strPasswd, std::string& OTP_Key)
{
	...
	if (result == 16)
	{
		g_pLoginMain->m_DBProcess.OTP(strAccountID, strPasswd1);
		unique_ptr<OdbcCommand> dbCommand2(m_dbConnection.CreateCommand());
		if (dbCommand2.get() == nullptr)
			return 17;
