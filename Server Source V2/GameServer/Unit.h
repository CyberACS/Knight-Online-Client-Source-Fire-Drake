...
public:
	...
	INLINE bool isInTempleEventZone(uint8 nZoneID = 0) 
	{
		...
		return nZoneID == ZONE_BORDER_DEFENSE_WAR || nZoneID == ZONE_CHAOS_DUNGEON || nZoneID == ZONE_JURAID_MOUNTAIN || nZoneID == ZONE_DEATHMATCH;
	}
	...
	INLINE bool DeathMatchEventZone(uint8 nZoneID = 0)
	{
		if (nZoneID == 0)
			nZoneID = GetZoneID();

		return nZoneID == ZONE_DEATHMATCH;
	}
