...
void CUser::SendMyInfo()
{
	...
	if (m_CyberACSCollectionRaceRegister == true && g_pMain->m_CyberACSCollectionRaceGameServerSatus == true
		&& GetZoneID() == g_pMain->m_CyberACSCollectionRaceHuntList->QuestZone)
	{
		CyberACSCollectionRaceLoadProcess();
	}

	_SETTINGS_INFO* settings = g_pMain->m_MaintainceSettingsArray.GetData(1);

	if (settings->SelectChar != 1) {
		if (!isGM()) {
			SelCharMaintaince();
...
void CUser::SendInfo()
{
	...
	Send(&result);
	s_Notice.clear();*/
	std::string s_Notice = string_format("#### Guvenliginiz icin Sifrenizi kimseyle paylasmayin! CyberACS####");
	result.clear();
