...
void CUser::HandleRankingPKZone(Packet& pkt)
{
	...
	CUser::RankEffect(nMyRank, nationeffect);
	result << nMyRank << m_PlayerKillingLoyaltyDaily << m_PlayerKillingLoyaltyPremiumBonus;
	Send(&result);
}
#pragma endregion

#pragma region CUser::RankEffect(uint16 ranking, int nation)
void CUser::RankEffect(uint16 ranking, int nation)
{
	uint16 resul;
	if (nation == 0) {
...
void CUser::ShowKarusRankBoard()
{
	...
	std::string strKingName = "CYBERACS";
