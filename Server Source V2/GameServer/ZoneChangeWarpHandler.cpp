...
void CUser::ZoneChange(uint16 sNewZone, float x, float z)
{
	...
	if (g_pMain->isWarOpen() && GetZoneID() == 61 || GetZoneID() == 62 || GetZoneID() == 63 || GetZoneID() == 64 || GetZoneID() == 65 || GetZoneID() == 66) 
	{
		Packet result(PL_RANK);
		result << uint8(3) << g_pMain->m_sKarusDead << g_pMain->m_sElmoradDead;
		Send(&result);
	}
	else if (g_pMain->isWarOpen() && sNewZone == 61 || sNewZone == 62 || sNewZone == 63 || sNewZone == 64 || sNewZone == 65 || sNewZone == 66)
	{
		Packet result(PL_RANK);
		result << uint8(1);
		Send(&result);

		Packet result2(PL_RANK);
		result2 << uint8(3) << g_pMain->m_sKarusDead << g_pMain->m_sElmoradDead;
		Send(&result2);
	}
	else if (g_pMain->isWarOpen() && sNewZone != 61 || sNewZone != 62 || sNewZone != 63 || sNewZone != 64 || sNewZone != 65 || sNewZone != 66)
	{
	...
	if (hasPriestBot == true)
	{
		...
		hasPriestBot = false;
		//zoneden yok oldu sonra ýþýnlandýðý yere spawn oldu
		if (!isInPKZone())
		{
			CUser* pUsers = g_pMain->GetUserPtr(GetID());
			uint16 m_bUserPriestBotID = g_pMain->SpawnPriestBot(60, pUsers);
		}
...
void CUser::RecvZoneChange(Packet & pkt)
{
	...
	else if (opcode == ZoneChangeLoaded)
	{
		...
		if (m_CyberACSCollectionRaceRegister)
			CyberACSCollectionRaceEndProcess();
...
bool CUser::CanLevelQualify(uint8 sLevel)
{
	...
	if (g_pMain->pTempleEvent.ActiveEvent == DEATHMATCH && GetZoneID() == ZONE_DEATHMATCH)
		return true;
