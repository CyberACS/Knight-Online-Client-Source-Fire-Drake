...
void CUser::ItemGet(Packet & pkt)
{
	...
	else
	{
		...
		uint32 transactionPrice = ((uint32)pTable->m_iBuyPrice);
		uint32 armorKinds[] = { 210, 220, 230, 240 };
		uint32 weaponKinds[] = { 11, 21, 22, 31, 32, 41, 42, 51, 52, 60, 70, 71, 80, 110, 120, 140 };

		if (m_lootSettings.iPrice > 0 && transactionPrice < m_lootSettings.iPrice)
			return;
		if (pTable->m_bCountable > 0 && m_lootSettings.iConsumable == 0)
			return;
		if (pTable->m_bClass == 6 && m_lootSettings.iWarrior == 0)
			return;
		if (pTable->m_bClass == 2 && m_lootSettings.iRogue == 0)
			return;
		if (pTable->m_bClass == 10 && m_lootSettings.iMage == 0)
			return;
		if (pTable->m_bClass == 4 && m_lootSettings.iPriest == 0)
			return;
		if (pTable->m_ItemType == 0 && m_lootSettings.iNormal == 0)
			return;
		if (pTable->m_ItemType == 1 && m_lootSettings.iMagic == 0)
			return;
		if (pTable->m_ItemType == 2 && m_lootSettings.iRare == 0)
			return;
		if (pTable->m_ItemType == 3 && m_lootSettings.iCraft == 0)
			return;
		if (pTable->m_ItemType == 4 && m_lootSettings.iUnique == 0)
			return;
		if (pTable->m_ItemType == 5 && m_lootSettings.iUpgrade == 0)
			return;

		if (m_lootSettings.iArmor == 0)
		{
			for (int i = 0; i < 4; i++)
				if (pTable->m_bKind == armorKinds[i])
					return;
		}

		if (m_lootSettings.iWeapon == 0)
		{
			for (int i = 0; i < 9; i++)
				if (pTable->m_bKind == weaponKinds[i])
					return;
		}
		...
		if (HasObtained == true)
		{
			_CYBERACS_SERVER_SETTINGS *pSettings = g_pMain->m_ServerSettingsArray.GetData(g_pMain->m_nServerNo);
