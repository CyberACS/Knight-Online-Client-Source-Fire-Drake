	...
	if (hook == 2)
	{
		for (int i = 0; i < 10; i++)
		{
			...
			if (m_bstrSkill[0] == 0)
			{
				...
				Packet result2(WIZ_SKILLPT_CHANGE, uint8(0));
				result2 << m_bstrSkill[0]; // only send the packet on failure
				Send(&result2);
		...
		if (m_bstrSkill[0] >= 1)
		{
			...
			Packet result2(WIZ_SKILLPT_CHANGE, uint8(0));
			result2 << m_bstrSkill[0] - 1; // only send the packet on failure
			Send(&result2);
		}
