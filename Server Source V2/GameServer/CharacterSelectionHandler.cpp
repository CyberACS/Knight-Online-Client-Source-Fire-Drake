...
void CUser::SelCharMaintaince()
{
	string title, message;
	title = "Error";
	message = "The game is in maintenance mode.";

	Packet result1(PL_MESSAGE2);
	result1 << title << message;
	Send(&result1);
}
		...
		if (!isGM())
		{
			...
			if (GetZoneID() == ZONE_BIFROST)
			{
				...
				if (g_pMain->pBeefEvent.isActive)
				{
					...
					{
						if (g_pMain->pBeefEvent.WictoryNation != GetNation())
						{
	...
	case 4:
	{
		...
		if (strCharID1.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID1, ZoneID))
			{
				...
				{
					result << uint8(PrisonCharacter) << strCharID1 << "CyberACS";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID2.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID2, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID2 << "CyberACS";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID3.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID3, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID3 << "CyberACS";
					Send(&result);
					return;
				}
			}
		...
		if (strCharID4.length() != 0)
		{
			if (g_DBAgent.CharacterSelectPrisonCheck(strCharID4, ZoneID))
			{
				if (ZoneID == ZONE_PRISON)
				{
					result << uint8(PrisonCharacter) << strCharID4 << "CyberACS";
					Send(&result);
					return;
				}
			}
