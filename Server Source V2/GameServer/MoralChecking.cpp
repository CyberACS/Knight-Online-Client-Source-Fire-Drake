...
bool CBot::isMoral2Checking(Unit* pTarget, _MAGIC_TABLE* pSkill)
{
	...
	if (pSkill->bMoral == MORAL_FRIEND_WITHME)
	{
				...
				pSkill->iNum == 212545)
			{
				...
				if (pTable != nullptr)
				{
					if (pTable->isDarkKnightMace())
					{
						...
						{
							if (isInMeleeArena() && TO_USERB(pTarget)->isInMeleeArena())
							{
								return false;
							}
							else if (isInPartyArena() && TO_USERB(pTarget)->isInPartyArena())
							{
								if (isInParty() && GetPartyID() != TO_USERB(pTarget)->GetPartyID())
									return false;
							...
							{
								if (isInClanArena() && TO_USERB(pTarget)->isInClanArena())
								{
								...
								if (isInEnemySafetyArea() && !TO_USERB(pTarget)->isInEnemySafetyArea())
									return true;
						...
						if (GetZoneID() == ZONE_DELOS)
						{
							...
							if (g_pMain->pCswEvent.Started)
							{
								...
								if (pKnightsSource != nullptr && pKnightsTarget != nullptr)
								{
									if (isInEnemySafetyArea()
										|| isInOwnSafetyArea()
										|| TO_USERB(pTarget)->isInEnemySafetyArea()
										|| TO_USERB(pTarget)->isInOwnSafetyArea())
									{
										if (pKnightsSource->GetID() == pKnightsTarget->GetID())
											return false;
									...
									else if (g_pMain->pCswEvent.Status == CswOperationStatus::CastellanWar)
									{
										...
										if (pKnightSiege != nullptr)
										{
											...
											if (pOldWinnerSource != nullptr)
											{
												...
												if (pInfo1 != nullptr && (GetClanID() != TO_USERB(pTarget)->GetClanID()))
													return false;
											}
											else
											{
		...
		if (isInArena() && TO_USERB(pTarget)->isInArena())
		{
			...
			else if (isInMeleeArena() && TO_USERB(pTarget)->isInMeleeArena())
				return true;
...
bool CBot::isMoral7Checking(Unit* pTarget, _MAGIC_TABLE* pSkill)
{
	// For non-player hostility checks, refer to the appropriate override.
	if (!pTarget->isPlayer())
		return pTarget->isMoral7Checking(this, pSkill);
