...
COMMAND_HANDLER(CUser::HandleMonsterSummonCommand)
{
	...
	int bType = 0;
	if (!vargs.empty())
		bType = atoi(vargs.front().c_str());

	//g_pMain->SpawnEventNpc(sSid, true, GetZoneID(), GetX(), GetY(), GetZ(), Count, 5, 0, 0, -1, GetEventRoom(), 0, bType);
	g_pMain->SpawnEventNpc(sSid, true, GetZoneID(), GetX(), GetY(), GetZ(), Count, 0, 3* SECOND);
