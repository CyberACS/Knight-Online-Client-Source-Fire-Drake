...
typedef std::map<uint32, ULONGLONG>			SkillCastimeList;
...
struct LOTTERY_USERS
{
	std::string	Name;
	uint16	TotalTicket;
	uint8	chance;
	LOTTERY_USERS(std::string Name1, uint16 TotalTicket1, uint8	chance1)
	{

		Name = Name1;
		TotalTicket = TotalTicket1;
		chance = chance1;
...
public:
	...
	//Lottery event
	uint16 PurchasedLottery = 0;
	std::string names[100];
	std::vector<LOTTERY_USERS> User;
	...
	/* DeathMatch */
	_ITEM_DATA m_sItemArrayBackup[INVENTORY_TOTAL];
	uint8	m_bRaceBackup;
	uint16	m_sClassBackup;
	uint8	m_bStatsBackup[STAT_COUNT];
	uint8    m_bstrSkillBackup[10];
	bool	m_sInDeathMatchEvent;
	...
	SkillCastimeList	m_SkillCastimeList;
...
public:
	...
	INLINE bool ÝsInApprovedParty() { return m_bInParty; }
	...
	INLINE bool isWantedUser() { return m_WantedSelectingUser; }
	...
	*/
	...
	INLINE void GetTotalSkill(SkillPointCategory category, int8 val)
	{
		if (category >= SkillPointCount)
		{
			return;
		}

		m_bstrSkill[category] = val;
	}
	...
	void HandleSurroundingUserInfoDetailV2(Packet& pkt);
	void HandleHookUser(Packet& pkt);
	...
	void PlayerEffect(uint16 sEffect);
	...
	void VSEventProcess(int Code);

	void SelCharMaintaince();
	...
	uint8 JobChangev2(Packet& pkt);
	void GenderChangeV2(Packet& pkt);

	/*CYBERACS System Entegrasyon*/
	void LifeSkillHuntProcess(uint16 MonsterNum);
	void CyberACS_Main();
	void CyberACS_SendLifeSkills();
	void CyberACS_HpInfo();
	void CyberACS_PowerUpStore();
	void CyberACS_ItemNotice(uint8 type, uint32 nItemID);
	void CyberACS_SendMessageBox(std::string title, std::string message);
	bool CyberACS_CashLose(uint32 cash);
	void CyberACS_CashGain(uint32 cash);
	void CyberACS_SendPartyProcess();
	void CyberACSCollectionRaceLoadProcess();
	void CyberACSCollectionRaceReward();
	void CyberACSCollectionRaceEndProcess();
	void CyberACSCollectionRaceProcess(uint16 MonsterNum);

	bool m_CyberACSCollectionRaceRegister;
	uint16 m_CyberACSCollectionRaceEventMonsterID[CR_EVENT_COUNT];
	uint16 m_CyberACSCollectionRaceEventMonsterKillCount[CR_EVENT_COUNT];
	uint16 m_CyberACSCollectionRaceEventCount[CR_EVENT_COUNT];
	uint8 m_CyberACSCollectionRaceEventZone[CR_EVENT_COUNT];
	std::string m_CyberACSCollectionRaceEventMonsterName[CR_EVENT_COUNT];

	//daily quest//
	bool m_DailyRegister[120];
	uint16 m_DailyEventMonsterID[4][120];
	uint16 m_DailyEventMonsterKillCount[4][120];
	uint16 m_DailyEventCount[4][120];
	uint8 m_DailyEventZone[4][120];
	std::string m_DailyEventMonsterName[4][120];
	void DailyLoadProcess(uint8 Quest);
	void DailyLoadProcessPacket();
	void DailyReward(int numara);
	void DailyEndProcess(int numara);
	void DailyProcess(uint16 MonsterNum);
	...
	time_t CyberACS_LASTCHECK;
	uint64 CyberACS_TICKCOUNT;
	time_t CyberACS_LASTSUPPORT;

	//sehzadeguard

	void SehzadeGuard(Packet& pkt);
	void InitiateGuard();
	void CheckSexyGuardAlive();
	uint32	m_tGuardAliveExpiryTime;
	uint32 m_tGuardInitiationTimer;
	bool isSexyGuardAlive;
	uint64 m_kCryptoKey;
	uint8 m_GuardTrial;

	// New Packet Hook
	void ReqSaveLootSettings(Packet& pkt);
	...
	_LOOT_SETTINGS m_lootSettings;

	/*CyberACS Packet System*/
	void HandleCyberACS(Packet& pkt);
	void CyberACSMonsterInfoHandler(Packet& pkt);
	void CyberACS_AuthInfo(Packet& pkt);
	void SlaveMerc(Packet& pkt);
	void DailyQuest(Packet& pkt);
	void CyberACS_General(Packet& pkt);
	void CyberACS_UIRequest(Packet& pkt);
	void CyberACS_ReqUserData(Packet& pkt);
	void CyberACS_StayAlive(Packet& pkt);
	void CyberACS_BotPriestSystem();
	void CyberACS_HandleLifeSkill(Packet& pkt);
	void CyberACS_ReqUserInfo(Packet& pkt);
	void CyberACS_Support(Packet& pkt);
	void CyberACS_ReqMerchantList(Packet& pkt);
	void CyberACS_SendTempItems(Packet& pkt);
	void CyberACS_DropRequest(Packet& pkt);
	void CyberACS_PowerUpStoreRequest(Packet& pkt);
	void CyberACS_PowerUpStorePurchase(Packet& pkt);
	void CyberACS_AllSkillStatRestard(Packet& pkt);
	void CyberACS_ChaoticExchange(Packet& pkt);
	void CyberACS_MerchantHandler(Packet& pkt);
	void CyberACS_MerchantItemAdd(Packet& pkt);
	void CyberACS_VoiceHandler(Packet& pkt);
	void CyberACS_LogHandler(Packet& pkt);
	void CyberACS_GMTOOLS(Packet& pkt);

	void MannerStore(Packet& pkt);
	...
	COMMAND_HANDLER(HandleGMtools);

	COMMAND_HANDLER(HandleDeathMatch);
	COMMAND_HANDLER(HandleCloseEvents);

	COMMAND_HANDLER(HandleSendCash);
	COMMAND_HANDLER(MaintainceSend);
	COMMAND_HANDLER(HandleTestUif);
	...
	void Password(Packet& pkt);
	void Ranks(Packet& pkt);
	void UserTools(Packet& pkt);
	void VsEvent(Packet& pkt);
	void DeathNotice(Packet& pkt);
	...
	void CYBACSClanWarehouseProcess(Packet& pkt);
	...
	// Ranking System
	// Ranking System
	void RankEffect(uint16 ranking, int nation);
	...
	void VanGuardProcess(Packet& pkt);
	void WantedEventUserisMove();
	void WantedEventUserisUpdate();
	void WantedEventisExits();
	void WantedEventDead();
	void WantedEventKilled();
	...
	uint8 TournumentRegister();
	uint8 TournumentCancelRegister();
...
public:
	...
	DECLARE_LUA_GETTER(TournumentCancelRegister)
	DECLARE_LUA_GETTER(TournumentRegister)
	...
	DECLARE_LUA_FUNCTION(JobChangeCagir) {
		LUA_NO_RETURN(LUA_GET_INSTANCE()->JobChangeCagir());
	}
